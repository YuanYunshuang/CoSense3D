...
from cosense3d.modules import cat_coor_with_idx
...
class SeqDataManager:
    ...
    def remove_empty_boxes(self):
        ...
        for i, seq_cavs in enumerate(self.cav_manager.cavs):
            for b, cavs in enumerate(seq_cavs):
                ...
                ego_cav.data[i]['global_labels_3d'] = ego_cav.data[i]['global_labels_3d'][m]
                ptr += 1
    ...
    def gather(self, task_list, data_keys):
        data_list = []
        for k in data_keys:
            data = []
            for task_id in task_list:
                cav_id, seq_idx = task_id.rsplit('.', 1)
                if k == 'memory':
                    v = self.cav_manager.get_cav_with_id(cav_id).memory
                else:
                    v = self.cav_manager.get_cav_with_id(cav_id).data[int(seq_idx)][k]
                data.append(v)
            data_list.append(data)
        return data_list

    def scatter(self, task_list, data_dict):
        for k, data_list in data_dict.items():
            for task_id, data in zip(task_list, data_list):
                cav_id, seq_idx = task_id.rsplit('.', 1)
                self.update(cav_id, k, data, int(seq_idx))

    def update(self, cav_id, data_key, data, seq_idx):
        self.cav_manager.get_cav_with_id(cav_id).data[seq_idx][data_key] = data
