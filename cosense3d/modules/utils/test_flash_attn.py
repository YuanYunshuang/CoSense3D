...
from flash_attn.flash_attn_interface import (flash_attn_func, flash_attn_unpadded_qkvpacked_func, \
     _get_block_size, flash_attn_unpadded_kvpacked_func, flash_attn_unpadded_func)
...
def generate_qkv(x, Wqkv, nheads, query_padding_mask=None, key_padding_mask=None,
    ...
    """
    :param x: (batch_size, seqlen, nheads * d)
    :param Wqkv: nn.Linear(nheads * d, 3 * nheads * d)
    :param query_padding_mask: (batch_size, seqlen), bool
    :param key_padding_mask: (batch_size, seqlen), bool
    """
...
def attention_ref(q, k, v, query_padding_mask=None, key_padding_mask=None, dropout_p=0.0,
    ...
    """
    :param q: (batch_size, seqlen_q, nheads, head_dim)
    :param k: (batch_size, seqlen_k, nheads, head_dim)
    :param v: (batch_size, seqlen_k, nheads, head_dim)
    :param query_padding_mask: (batch_size, seqlen_q)
    :param key_padding_mask: (batch_size, seqlen_k)
    :param dropout_p: float
    :param dropout_mask: (batch_size, nheads, seqlen_q, seqlen_k)
    :param bias: (batch_size, nheads, seqlen_q, seqlen_k)
    :param upcast: whether to cast all inputs to fp32, do all computation in fp32, then cast
        output back to fp16/bf16.
    :param reorder_ops: whether to change the order of operations (scaling k instead of scaling k, etc.)
...
def test_flash_attn_unpadded_qkvpacked(seqlen, d, dropout_p, causal, dtype):
    if seqlen >= 2048 and torch.cuda.get_device_properties('cuda').total_memory <= 16 * 2**30:
        pass  # Reference implementation OOM
    device = 'cuda'
...
def test_flash_attn_unpadded_kvpacked(seqlen_q, seqlen_k, d, dropout_p, causal, dtype):
    ...
    batch_size = 5
    nheads = 8
    x = torch.randn(batch_size, seqlen_k, nheads * d, device=device, dtype=dtype, requires_grad=True)
    Wqkv = torch.nn.Linear(nheads * d, 3 * nheads * d, device=device, dtype=dtype)
...
if __name__=="__main__":
    test_flash_attn_unpadded_kvpacked(900, 1280,16, 0, False, torch.float16)