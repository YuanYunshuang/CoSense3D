...
#--------- Ablation 5 : No dilconv ------------
shared_modules_opv2vt_no_dilconv = get_shared_modules(opv2vt.point_cloud_range, opv2vt.global_ref_time, 32)
shared_modules_opv2vt_no_dilconv['backbone_neck'] = dict(
            type='necks.dilation_spconv.DilationSpconvAblation',
            gather_keys=['bev_feat'],
            scatter_keys=['bev_feat'],
            data_info=dict(lidar_range=opv2vt.point_cloud_range, voxel_size=voxel_size),
            d=2,
            convs=dict(
                p2=dict(kernels=[1], in_dim=384, out_dim=256),
                p8=dict(kernels=[1], in_dim=256, out_dim=256),
            )
        )
...
#--------- Comparative 3 : Latency with box roi ------------
shared_modules_dairv2xt_bev_fcl_lat = copy.deepcopy(shared_modules_dairv2xt)

#--------- Ablation 5 : No dilconv ------------
shared_modules_dairv2xt_no_dilconv = copy.deepcopy(shared_modules_dairv2xt)
shared_modules_dairv2xt_no_dilconv['backbone_neck'] = dict(
            type='necks.dilation_spconv.DilationSpconvAblation',
            gather_keys=['bev_feat'],
            scatter_keys=['bev_feat'],
            data_info=dict(lidar_range=dairv2xt.point_cloud_range, voxel_size=voxel_size),
