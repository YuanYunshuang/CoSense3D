...
def sigmoid_focal_classification_loss(preds, tgts, n_cls, weights=None, reduction='none',
                                      gamma=2.0, alpha=0.25):
    pred_sigmoid = torch.sigmoid(preds)
    alpha_weight = tgts * alpha + (1 - tgts) * (1 - alpha)
    pt = tgts * (1.0 - pred_sigmoid) + (1.0 - tgts) * pred_sigmoid
    focal_weight = alpha_weight * torch.pow(pt, gamma)

    bce_loss = torch.clamp(preds, min=0) - preds * tgts + \
               torch.log1p(torch.exp(-torch.abs(preds)))

    loss = focal_weight * bce_loss

    if weights.shape.__len__() == 2 or \
            (weights.shape.__len__() == 1 and tgts.shape.__len__() == 2):
        weights = weights.unsqueeze(-1)
