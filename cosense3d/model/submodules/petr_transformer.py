...
from cosense3d.utils.module_utils import instantiate_target_module, build_norm_layer, build_dropout
from cosense3d.model.utils import xavier_init
from cosense3d.model.submodules.attention import FlashMHA, MultiheadAttention, FFN
...
class PETRTransformerEncoder(nn.Module):
...
class PETRTransformerDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            self.layers.append(
                PETRTemporalDecoderLayer(attn_cfgs,
                                         ffn_cfgs,
                                         with_cp=with_cp,
        ...
        if post_norm_cfg is not None:
            self.post_norm = build_norm_layer(post_norm_cfg, self.embed_dims)
        else:
            self.post_norm = None
...
class PETRTemporalTransformer(nn.Module):
    ...
    def __init__(self, cfg, cross=False):
        ...
        decoder_args = self.reformat_decoder_args(cfg['decoder']['args'])
        self.decoder = instantiate_target_module(cfg['decoder']['_target_'],
                                                 **decoder_args)
        self.embed_dims = self.decoder.embed_dims
        self.cross = cross

    def reformat_decoder_args(self, decoder_args):
        embed_dims = decoder_args['embed_dims']
        num_heads = decoder_args['num_heads']
        attn_dropout = decoder_args['attn_dropout']
        decoder_args['attn_cfgs'] = [{'type': 'MultiheadAttention',
                                     ...
                                     {'type': 'PETRMultiheadFlashAttention',
                                      ...
                                      'num_heads': num_heads,
                                      'dropout': attn_dropout}]
        decoder_args['ffn_cfgs'] = dict(
                     type='FFN',
                     embed_dims=embed_dims,
                     feedforward_channels=decoder_args['feedforward_channels'],
                     num_fcs=2,
                     ffn_drop=decoder_args.get('ffn_dropout', 0.),
...
class PETRTemporalDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        for operation_name in operation_order:
            if operation_name in ['self_attn', 'cross_attn']:
                ...
                _cfgs = copy.copy(attn_cfgs[index])
                attn_type = _cfgs.pop('type')
                attention = globals().get(attn_type)(**_cfgs)
