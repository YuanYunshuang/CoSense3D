...
class Encoder(nn.Module):
    def __init__(self, cfgs):
        super().__init__()
        self.cfgs = cfgs
        sp = self.cfgs['img_shape']
        dim = self.cfgs['dim']
        cross_view = self.cfgs['cross_view']
        bev_embedding = self.cfgs['bev_embedding']
        self.feature_shapes = [(32, sp[0] // 4, sp[1] // 4),
                               (112, sp[0] // 16, sp[1] // 16)]

        cross_views = list()
        layers = list()

        for feat_shape, num_layers in zip(self.feature_shapes, self.cfgs['middle']):
            feat_dim, feat_height, feat_width = feat_shape
    ...
    def forward(self, batch_dict):
        b, n, _, _, _ = batch_dict['imgs'].shape

        I_inv = batch_dict['cam_intrinsics'].inverse()           # b n 3 3
        E_inv = batch_dict['cam_extrinsics'].inverse()           # b n 4 4

        features = batch_dict['backbone_2d']
