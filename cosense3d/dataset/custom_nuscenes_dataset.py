...
class CustomNuscenesDataset(NuScenesDataset):
    ...
    def __init__(self, cfgs, mode='train'):
        ...
        self.mode = mode
        data_cfg = dict(
            collect_keys = ['lidar2img', 'intrinsics', 'extrinsics', 'timestamp', 'img_timestamp', 'ego_pose',
                           'ego_pose_inv', 'img', 'prev_exists', 'img_metas'],
            seq_mode= False,
            seq_split_num= 1,
            num_frame_losses= 1,
            queue_length= 4,
            random_length= 1,
            data_root= '/koko/nuScenes/keyframes/',
            ann_file= '/koko/nuScenes/keyframes/nuscenes2d_temporal_infos_train.pkl',
            classes = ['car', 'truck', 'construction_vehicle', 'bus', 'trailer', 'barrier', 'motorcycle', 'bicycle', 'pedestrian', 'traffic_cone'],
            modality= {'use_lidar': False, 'use_camera': True, 'use_radar': False, 'use_map': False, 'use_external': True},
            test_mode=False,
            box_type_3d='LiDAR',
            use_valid_flag=True,
            filter_empty_gt=False,
            pipeline=[{'type': 'LoadMultiViewImageFromFiles', 'to_float32': True},
                      {'type': 'LoadAnnotations3D', 'with_bbox_3d': True, 'with_label_3d': True, 'with_bbox': True, 'with_label': True, 'with_bbox_depth': True},
                      {'type': 'ObjectRangeFilter', 'point_cloud_range': [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0]},
                      ...
                      {'type': 'ResizeCropFlipRotImage', 'data_aug_conf': {'resize_lim': (0.38, 0.55), 'final_dim': (256, 704), 'bot_pct_lim': (0.0, 0.0), 'rot_lim': (0.0, 0.0), 'H': 900, 'W': 1600, 'rand_flip': True}, 'training': True},
                      {'type': 'GlobalRotScaleTransImage', 'rot_range': [-0.3925, 0.3925], 'translation_std': [0, 0, 0], 'scale_ratio_range': [0.95, 1.05], 'reverse_angle': True, 'training': True},
                      {'type': 'NormalizeMultiviewImage', 'mean': [123.675, 116.28, 103.53], 'std': [58.395, 57.12, 57.375], 'to_rgb': True},
                      {'type': 'PadMultiViewImage', 'size_divisor': 32},
                      {'type': 'PETRFormatBundle3D',
                       'class_names': ['car', 'truck', 'construction_vehicle', 'bus', 'trailer', 'barrier', 'motorcycle', 'bicycle', 'pedestrian', 'traffic_cone'],
                       'collect_keys': ['lidar2img', 'intrinsics', 'extrinsics', 'timestamp', 'img_timestamp', 'ego_pose', 'ego_pose_inv', 'prev_exists']},
                      {'type': 'Collect3D',
                       'keys': ['gt_bboxes_3d', 'gt_labels_3d', 'img', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths', 'prev_exists', 'lidar2img', 'intrinsics', 'extrinsics', 'timestamp', 'img_timestamp', 'ego_pose', 'ego_pose_inv'],
        ...
        self.parse_args(**data_cfg)

    def parse_args(self, collect_keys, seq_mode=False, seq_split_num=1,
                   num_frame_losses=1, queue_length=8, random_length=0, *args, **kwargs):
    ...
    def collate_batch(batch_list):
        ...
        for k in batch_list[0].keys():
            ret[k] = [batch[k].data for batch in batch_list]
        keys_stack = ['img', 'prev_exists', 'lidar2img', 'intrinsics', 'extrinsics', 'timestamp', 'img_timestamp', 'ego_pose', 'ego_pose_inv']
        for k in keys_stack:
            ret[k] = torch.stack(ret[k], dim=0)
        keys_zip = ['gt_bboxes_3d', 'gt_labels_3d', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths', 'img_metas']
        for k in keys_zip:
            ret[k] = list(zip(*ret[k]))

        ret['batch_size'] = ret['img'].shape[0]
        ret['seq_len'] = ret['img'].shape[1]
        return ret
